...
- Shared-password login for non-Replit deployments: set `SITE_PASSWORD` (and optionally `SITE_USER_ID/EMAIL/FIRST_NAME/LAST_NAME`); `/api/login` then serves a password form and everyone signs in as one CRM user. Takes precedence over `LOCAL_DEV_AUTH`; Replit OIDC is used when neither is set.
- Vercel: `vercel.json` builds the frontend to `artifacts/crm/dist/public` and serves the API from `api/index.mjs`, which loads the self-contained `artifacts/api-server/dist/vercel.mjs` bundle (pdfkit + fonts + product JSON copied into `dist/` by `build.mjs`). Required Vercel env: `DATABASE_URL` (Supabase transaction pooler URL), `SITE_PASSWORD`, `ANTHROPIC_API_KEY`. Vercel builds via `scripts/vercel-build.sh`, which runs `pnpm --filter @workspace/db run push-force` so schema (including `sessions`) is applied on each Production deploy.
- DB pool: app sets `prepare: false` on the pg Pool for Supabase transaction pooler (PgBouncer) compatibility.
