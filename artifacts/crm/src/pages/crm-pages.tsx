...
function LeadQuotations({ leadId }: { leadId: number }) {
  const query = useListLeadQuotations(leadId, { query: { enabled: Number.isFinite(leadId), queryKey: getListLeadQuotationsQueryKey(leadId) } });
  const quotations = query.data ?? [];
  return <section className="rounded-xl border border-border bg-card p-5 md:p-6" data-testid="section-quotations"><div className="mb-5 flex items-center justify-between"><SectionLabel count={quotations.length}>Quotations</SectionLabel><Link href={`/enquiry?leadId=${leadId}`} data-testid="link-new-quotation" className="text-xs font-semibold text-primary hover:underline">New quotation</Link></div>{query.isLoading ? <SkeletonBlock className="h-12 w-full" /> : query.isError ? <p className="text-xs text-muted-foreground">Quotations could not be loaded.</p> : quotations.length ? <div className="divide-y divide-border">{quotations.map((quotation) => <div key={quotation.id} className="grid gap-2 py-3 sm:grid-cols-[1.2fr_1fr_1fr_auto_auto] sm:items-center" data-testid={`row-quotation-${quotation.id}`}><p className="font-mono text-xs font-bold">{quotation.quotationNumber}</p><p className="text-xs text-muted-foreground"><DateLabel value={quotation.createdAt.slice(0, 10)} /></p><p className="font-mono text-xs font-semibold">{formatInr(quotation.total)}</p><span className={`status-pill ${quotation.status === 'Generated' ? 'status-won' : 'status-proposal'}`}>{quotation.status}</span><a href={getGetQuotationPdfUrl(quotation.id)} target="_blank" rel="noreferrer" data-testid={`link-quotation-pdf-${quotation.id}`} className="inline-flex items-center gap-1 text-xs font-semibold text-primary hover:underline"><FileText className="size-3.5" />View PDF</a></div>)}</div> : <p className="text-xs text-muted-foreground">No quotations yet. Start one from “New quotation”.</p>}</section>;
}
